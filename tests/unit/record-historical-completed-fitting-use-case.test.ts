...
import {
  recordHistoricalCompletedFittingForTenant,
} from "../../src/fitting/application/use-cases/record-historical-completed-fitting";
import type {
  GarmentRepository,
...
describe(
  ...
  () => {
    ...
    it(
      "atomically creates an unscheduled session and completed outcome",
      async () => {
        const deps =
          dependencies();

        await recordHistoricalCompletedFittingForTenant(
    ...
    it(
      "creates neither record when no historical approved baseline existed",
      async () => {
        const deps =
          dependencies([]);
